...
def read_query():
    ...
    sql15 = '''
               exec USP_Auto_DailyCostsOfPackaging
               
            '''
    df15 = pd.read_sql_query(sql15, conn)

    sql18 = '''
               exec USP_Auto_DailyCostsOfGlazing

            '''

    df18 = pd.read_sql_query(sql18, conn)
    ...
    df15.rename(columns = {'ShippingDate':'OrderDate','PackageCost':'Packaging'}, inplace = True)
    
    df_con = df_con.merge(df15, on = 'OrderDate', how = 'inner')

    df_con = df_con.merge(df18, on = 'OrderDate', how = 'inner')
    
    add = ['CLs & Solutions','Sunglasses' ,'Glasses', 'Deliveries', 'Packaging', 'GlazingCost']

    df_con['Total'] = df_con[add].sum(axis = 1)
